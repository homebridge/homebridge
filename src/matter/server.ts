...
export class MatterServer extends EventEmitter {
  ...
  // State monitoring flag (for UI accessories page)
  ...
  constructor(config: MatterServerConfig) {
    ...
    // Format uniqueId as MAC address for username
    ...
    // Create behavior registry for this server instance
    // Pass 'this' server instance for state change notifications
    // Note: Endpoints are registered with behaviors when accessories are added (in registerAccessoryHandlers)
    this.behaviorRegistry = new BehaviorRegistry(this.accessories, this)

    // Create RegistryManager instance for this server
    // This manages the mapping between endpoints and their behavior registries
    this.registryManager = new RegistryManager()

    // Endpoints will be registered with the RegistryManager when they are created
  }
  ...
  private async registerAccessory(pluginIdentifier: string, platformName: string, accessory: MatterAccessory): Promise<void> {
    ...
    try {
      ...
      // Attach RegistryManager to endpoint
      setRegistryManager(endpoint, this.registryManager)
  ...
  private registerAccessoryHandlers(accessory: MatterAccessory): void {
    ...
    // Register this endpoint's BehaviorRegistry with the RegistryManager
    // Each MatterServer instance has its own RegistryManager
    this.registryManager.registerEndpoint(accessory.UUID, this.behaviorRegistry)
  ...
  private async createAccessoryParts(
    ...
    for (const part of accessory.parts) {
      ...
      // Attach RegistryManager to part endpoint
      setRegistryManager(partEndpoint, this.registryManager)
      ...
      if (part.handlers) {
        // Register this part endpoint's BehaviorRegistry with the RegistryManager
        this.registryManager.registerEndpoint(partEndpointId, this.behaviorRegistry)
  ...
  private async finalizeAccessoryRegistration(
    ...
    // Notify UI that accessories list has changed (for real-time updates)
   ...
   * This method is used by plugins to report physical device state changes to Homebridge
   *
   * Note: State updates are automatically deferred using setImmediate to avoid
   * "read-only transaction" errors when called from within Matter.js handler callbacks.
   *
   * @param uuid - Unique identifier of the accessory
   * @param cluster - Name of the Matter cluster to update (e.g., 'onOff', 'levelControl')
   * @param attributes - Cluster attributes to update as key-value pairs
   * @param partId - Optional part identifier for composed devices with multiple endpoints
   * @throws {MatterDeviceError} If the accessory or part is not found
   */
    ...
    return new Promise((resolve, reject) => {
      setImmediate(async () => {
        try {
          ...
          // Notify UI of state change (if monitoring is enabled)
  ...
  /**
   * Trigger a command on a Matter accessory (invokes user handlers)
   *
   * This method triggers actual commands on the cluster (e.g., "on", "off", "toggle" for OnOff cluster)
   * which executes the plugin's command handlers. This is the equivalent of HAP's setCharacteristic().
   *
   * Use this for UI/external control that should invoke plugin handlers.
   * Use updateAccessoryState() for state updates from the physical device.
   *
   * @param uuid - Accessory UUID
   * @param cluster - Cluster name (e.g., 'onOff', 'levelControl')
   * @param command - Command name (e.g., 'on', 'off', 'toggle', 'moveToLevel')
   * @param args - Command arguments (if any)
   * @param partId - Optional: ID of the part to control (for composed devices)
   */
  async triggerCommand(
    ...
    // Determine which endpoint to control
    let targetEndpoint: any
    let displayName: string

    if (partId) {
      // Control a specific part
      const part = accessory._parts?.find(p => p.id === partId)
      if (!part || !part.endpoint) {
        throw new MatterDeviceError(`Part ${partId} not found in accessory ${uuid}`)
      }
    ...
    // Execute the command using Matter.js's act() method
    // This properly invokes the command through the behavior system
    try {
      const partInfo = partId ? ` (part: ${partId})` : ''
      log.debug(`Triggering command ${cluster}.${command} for ${displayName}${partInfo}`, args)

      // Use act() to invoke the command through Matter.js' command handling
      // This will trigger our custom behavior classes which call the user handlers
      await targetEndpoint.act((agent: any) => {
        ...
        // Call the command with args if provided
  ...
  /**
   * Enable state monitoring - allows sending IPC updates when state changes
   * Called when UI accessories page is opened
   */
  ...
  /**
   * Disable state monitoring - stops sending IPC updates
   * Called when UI accessories page is closed
   */
   ...
   * This is called from BehaviorRegistry when cluster state updates, and from updateAccessoryState
   * when plugins report physical device changes. Only sends IPC updates if monitoring is enabled.
   *
   * @param uuid - Unique identifier of the accessory
   * @param cluster - Name of the Matter cluster that changed (e.g., 'onOff', 'levelControl')
   * @param state - Updated cluster attribute values
   * @param partId - Optional part identifier for composed devices
   */
  notifyStateChange(uuid: string, cluster: string, state: Record<string, unknown>, partId?: string): void {
    // Only send IPC updates if monitoring is active
    if (!this.monitoringEnabled) {
      return
    }

    // Emit event for listeners (managers can subscribe and forward via IPC)
    this.emit('stateChange', { uuid, cluster, state, partId })
  }

  /**
   * Extract device type name from EndpointType
   * @param deviceType - Matter.js EndpointType object
   * @returns Device type name as string (e.g. 'OnOffLight', 'DimmableLight')
   */
  private getDeviceTypeName(deviceType: EndpointType): string {
    // EndpointType has a name property (e.g. 'OnOffLight', 'DimmableLight')
    return (deviceType as any).name || 'Unknown'
  }

  /**
   * Collect all accessories for UI display
   * Used by MatterBridgeManager and ChildBridgeMatterManager
   */
