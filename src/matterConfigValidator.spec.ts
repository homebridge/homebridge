    ...
    it('should fail validation for weak passcodes', () => {
      ...
      for (const passcode of weakPasscodes) {
        const config: MatterConfiguration = {
          ...
          port: 5540,
          discriminator: 1234,
          passcode: typeof passcode === 'string' ? Number.parseInt(passcode) : passcode,
          vendorId: 0x1234,
          productId: 0x5678,
          deviceName: 'Test',
        }
        ...
        expect(result.isValid).toBe(false)
        expect(result.errors.some(e => e.includes('invalid') || e.includes('not allowed') || e.includes('weak') || e.includes('too weak'))).toBe(true)
      }
    })
