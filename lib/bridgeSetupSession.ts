...
export class BridgeSetupSession extends EventEmitter {
  ...
  protected stateChar: string;
  protected controlChar: string;
  ...
  private currentPlatformInstance: any;
  private currentPlatformContext: any = {};

  constructor(stateChar: string, controlChar: string) {
    super();
  ...
  pluginResponseHandler(response, type, replace, config) {
    if (config) {
  ...
  presentManagePlatformMenu() {
    ...
    for (const name in this.configurablePlatformPlugins) {
