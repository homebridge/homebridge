...
interface NegotiationRequest extends BaseRequest {
    language: string;
    type: 'Negotiate';
}

interface InterfaceRequest extends BaseRequest {
    response: Response;
    type: 'Interface';
}

interface TerminateRequest extends BaseRequest {
    type: 'Terminate';
}

type Request = NegotiationRequest | InterfaceRequest | TerminateRequest;

export class BridgeSetupSession extends EventEmitter {

    validSession: boolean = false;
    sessionUUID: string = uuid.generate(crypto.randomBytes(32).toString());
    transactionID: number = 0;
    platformContext?: PlatformContext;
    preferedLanguage: string = "en-US";
    lastResponse: string | null = null;
    listOfPlatforms: string[] = [];
    ...
    handleWriteRequest = (request: Request) => {
        if (request.type === "Negotiate") {
            ...
            if (this.currentStage === 1) {
                ...
                const targetPlatform = this.configurablePlatformPlugins[targetPlatformName];
                ...
                this.currentStage = 3;
                this.currentPlatformInstance.configurationRequestHandler(this.currentPlatformContext, null, this.pluginResponseHandler);
            }
            else if (this.currentStage === 3) {
                this.currentPlatformInstance.configurationRequestHandler(this.currentPlatformContext, request, this.pluginResponseHandler);
            }
    ...
    pluginResponseHandler = (response: Request, type: any, replace: any, config: any) => {
        if (config) {
    ...
    presentManageAccessoryMenu = () => {
        this.emit('requestCurrentConfig', (config: Config) => {
    ...
    handleManageAccessory(request: Request) {
        this.presentMainMenu();
    }
