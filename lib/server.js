...
Server.prototype._publish = function() {
  ...
  info.setCharacteristic(Characteristic.Manufacturer, bridgeConfig.manufacturer || `${toTitleCase(require('../package.json').author.name)}`);
  info.setCharacteristic(Characteristic.Model, bridgeConfig.model || `${toTitleCase(require('../package.json').name)}`);
  info.setCharacteristic(Characteristic.SerialNumber, bridgeConfig.username);
  info.setCharacteristic(Characteristic.FirmwareRevision, bridgeConfig.firmware || `${toTitleCase(require('../package.json').version)}`);
