...
Server.prototype._handleUpdatePlatformAccessories = function() {
...
Server.prototype._handleNewConfig = function(type, name, replace, config) {
  if (type === "accessory") {
    ...
    if (!replace) {
      ...
      var targetName; // eslint-disable-line no-redeclare
      if (name.indexOf('.') !== -1) {
        targetName = name.split(".")[1];
      }

      var found = false; // eslint-disable-line no-redeclare
      for (var index in this._config.platforms) {  // eslint-disable-line no-redeclare
