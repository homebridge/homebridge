...
class PluginManager {
    ...
    constructor(api, options) {
        ...
        if (options) {
            if (options.customPluginPath) {
                this.searchPaths.add((0, node_path_1.resolve)(node_process_1.default.cwd(), options.customPluginPath));
            }
    ...
    loadInstalledPlugins() {
        ...
        this.searchPaths.forEach((searchPath) => {
            if (!(0, node_fs_1.existsSync)(searchPath)) { // just because this path is in require.main.paths doesn't mean it necessarily exists!
                return;
            }
            if ((0, node_fs_1.existsSync)((0, node_path_1.join)(searchPath, 'package.json'))) { // does this path point inside a single plugin and not a directory containing plugins?
                try {
                ...
                const relativePluginPaths = (0, node_fs_1.readdirSync)(searchPath) // search for directories only
                    .filter((relativePath) => {
                    try {
                        return (0, node_fs_1.statSync)((0, node_path_1.resolve)(searchPath, relativePath)).isDirectory();
                    }
                    catch (error) {
                        log.debug(`Ignoring path ${(0, node_path_1.resolve)(searchPath, relativePath)} - ${error.message}`);
                        return false;
                    }
                });
                ...
                relativePluginPaths.slice()
                    ...
                    const absolutePath = (0, node_path_1.join)(searchPath, scopeDirectory);
                    (0, node_fs_1.readdirSync)(absolutePath)
                        .filter(name => PluginManager.isQualifiedPluginIdentifier(name))
                        .filter((name) => {
                        try {
                            return (0, node_fs_1.statSync)((0, node_path_1.resolve)(absolutePath, name)).isDirectory();
                        }
                        catch (error) {
                            log.debug(`Ignoring path ${(0, node_path_1.resolve)(absolutePath, name)} - ${error.message}`);
                            return false;
                        }
                    })
                ...
                relativePluginPaths
                    ...
                    try {
                        const absolutePath = (0, node_path_1.resolve)(searchPath, pluginIdentifier);
    ...
    loadPlugin(absolutePath) {
        ...
        const plugin = new plugin_js_1.Plugin(name, absolutePath, packageJson, scope);
        this.plugins.set(identifier, plugin);
        return plugin;
    }
    static loadPackageJSON(pluginPath) {
        const packageJsonPath = (0, node_path_1.join)(pluginPath, 'package.json');
        let packageJson;
        if (!(0, node_fs_1.existsSync)(packageJsonPath)) {
            throw new Error(`Plugin ${pluginPath} does not contain a package.json.`);
        }
        try {
            packageJson = JSON.parse((0, node_fs_1.readFileSync)(packageJsonPath, { encoding: 'utf8' })); // attempt to parse package.json
        }
        catch (error) {
    ...
    addNpmPrefixToSearchPaths() {
        if (node_process_1.default.platform === 'win32') {
            this.searchPaths.add((0, node_path_1.join)(node_process_1.default.env.APPDATA, 'npm/node_modules'));
        }
        else {
            this.searchPaths.add((0, node_child_process_1.execSync)('/bin/echo -n "$(npm -g prefix)/lib/node_modules"', {
                env: Object.assign({
     ...
     */
    async reloadPlugin(pluginIdentifier) {
