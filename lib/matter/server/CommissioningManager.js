...
exports.CommissioningManager = void 0;
const node_crypto_1 = require("node:crypto");
const promises_1 = require("node:fs/promises");
const node_path_1 = require("node:path");
const schema_1 = require("@matter/types/schema");
...
class CommissioningManager {
    ...
    commissioningInfo = {};
    constructor() {
        this.vendorId = ServerConfig_js_1.DEFAULT_VENDOR_ID;
        this.productId = ServerConfig_js_1.DEFAULT_PRODUCT_ID;
    }
    /**
    ...
    generateSecurePasscode() {
        let passcode;
        const maxAttempts = ServerConfig_js_1.MAX_PASSCODE_ATTEMPTS;
        let attempts = 0;
        ...
        do {
            ...
            let randomValue;
            do {
                randomValue = (0, node_crypto_1.randomBytes)(4).readUInt32BE(0);
    ...
    generateRandomDiscriminator() {
        ...
        const discriminator = (0, node_crypto_1.randomBytes)(2).readUInt16BE(0) & 0x0FFF; // Mask to 12 bits
    ...
    async loadOrGenerateCredentials(matterStoragePath) {
        const credentialsPath = (0, node_path_1.join)(matterStoragePath, 'credentials.json');
        try {
        ...
        await (0, promises_1.writeFile)(credentialsPath, JSON.stringify({ passcode: this.passcode, discriminator: this.discriminator }, null, 2), 'utf-8');
    ...
    async generateCommissioningInfo(deps) {
        ...
        const manualCode = schema_1.ManualPairingCodeCodec.encode({
            discriminator,
        ...
        const qrCodePayload = schema_1.QrPairingCodeCodec.encode([{
                version: 0,
                vendorId,
                productId,
        ...
        try {
            ...
            const commissioningFilePath = (0, node_path_1.join)(deps.matterStoragePath, 'commissioning.json');
            ...
            await (0, promises_1.writeFile)(commissioningFilePath, JSON.stringify(commissioningData, null, 2), 'utf-8');
        ...
        const qrCodeString = schema_1.QrCode.get(qrCodePayload);
        log.info(`\n${qrCodeString}`);
        log.info(`${'='.repeat(60)}`);
    }
    /**
...
exports.CommissioningManager = CommissioningManager;
