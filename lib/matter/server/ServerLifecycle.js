...
class ServerLifecycle {
    ...
    async createServerNodeWithRecovery(nodeOptions, sanitizedId) {
        try {
            return await main_1.ServerNode.create(nodeOptions);
        }
        catch (error) {
            ...
            const serverNodeStorePath = (0, node_path_1.join)(storageLocation, sanitizedId);
            const serverNodeStoreJsonFile = `${serverNodeStorePath}.json`;
            try {
                let removedSomething = false;
                // Delete the ServerNodeStore subdirectory
                try {
                    await (0, promises_1.stat)(serverNodeStorePath);
                    log.info(`Removing corrupted ServerNodeStore directory: ${serverNodeStorePath}`);
                    await (0, promises_1.rm)(serverNodeStorePath, { recursive: true, force: true });
                ...
                try {
                    await (0, promises_1.stat)(serverNodeStoreJsonFile);
                    log.info(`Removing corrupted ServerNodeStore JSON file: ${serverNodeStoreJsonFile}`);
                    await (0, promises_1.rm)(serverNodeStoreJsonFile, { force: true });
                    removedSomething = true;
                }
                catch (err) {
                    const code = err instanceof Error && 'code' in err ? err.code : undefined;
                ...
                const serverNode = await main_1.ServerNode.create(nodeOptions);
    ...
    async setupStorage(config) {
        ...
        const storagePath = (0, node_path_1.resolve)(config.storagePath);
        const normalizedPath = (0, node_path_1.normalize)(storagePath);
        // Ensure path is within allowed directories
        const allowedBasePaths = [
            (0, node_path_1.resolve)((0, node_os_1.homedir)(), '.homebridge'),
            (0, node_path_1.resolve)(node_process_1.default.cwd()),
            '/var/lib/homebridge',
        ];
        ...
        try {
            await (0, promises_1.mkdir)(normalizedPath, { recursive: true });
            await (0, promises_1.access)(normalizedPath, node_fs_1.constants.R_OK | node_fs_1.constants.W_OK);
        }
        catch (error) {
        ...
        this.matterStoragePath = (0, node_path_1.join)(normalizedPath, bridgeId);
        await (0, promises_1.mkdir)(this.matterStoragePath, { recursive: true });
        ...
        const { MatterAccessoryCache } = await Promise.resolve().then(() => __importStar(require('../accessoryCache.js')));
        ...
        try {
            ...
            const nodeOptions = {
                ...
                basicInformation: {
                    ...
                    vendorId: (0, main_1.VendorId)(deps.commissioningManager.vendorId),
                    ...
                    hardwareVersion: 1,
                    hardwareVersionString: (0, node_os_1.release)(),
                    softwareVersion: 1,
                    softwareVersionString: deps.config.firmwareRevision || (0, version_js_1.default)(),
                    reachable: true,
                },
            };
            ...
            if (!deps.config.externalAccessory) {
                const aggregator = new main_1.Endpoint(endpoints_1.AggregatorEndpoint, {
            ...
            node_process_1.default.on('SIGINT', shutdownHandler);
            node_process_1.default.on('SIGTERM', shutdownHandler);
            if (!deps.config.externalAccessory) {
    ...
    async runServer(deps) {
        ...
        if (!serverNode) {
            throw new types_js_1.MatterDeviceError('Server node not initialized - call start() first');
        }
        if (deps.getIsRunning()) {
            log.warn('Matter server is already running');
            return;
        ...
        if (!deps.config.externalAccessory) {
            throw new types_js_1.MatterDeviceError('runServer() should only be called when externalAccessory mode is enabled');
        }
    ...
    async startServerNode(serverNode, deps) {
        serverNode.run().then(() => {
            ...
            errorHandler_js_1.errorHandler.handleError(error, 'server-runtime');
        });
     ...
     */
    async waitForServerReady(deps, maxWaitTime = ServerConfig_js_1.SERVER_READY_TIMEOUT_MS) {
        const startTime = Date.now();
        while (!deps.getServerNode() || (!deps.config.externalAccessory && !deps.getAggregator())) {
            if (Date.now() - startTime > maxWaitTime) {
                throw new Error('Server failed to become ready within timeout');
            }
            await new Promise(resolve => setTimeout(resolve, ServerConfig_js_1.SERVER_READY_POLL_INTERVAL_MS));
        }
        await new Promise(resolve => setTimeout(resolve, ServerConfig_js_1.SERVER_INIT_DELAY_MS));
    }
    /**
    ...
    async stop(deps, accessories) {
        ...
        try {
            ...
            await errorHandler_js_1.errorHandler.handleError(error, 'server-stop');
            throw error;
        }
        finally {
            deps.setIsRunning(false);
    ...
    async cleanup(deps) {
        if (deps.shutdownHandler) {
            node_process_1.default.off('SIGINT', deps.shutdownHandler);
            node_process_1.default.off('SIGTERM', deps.shutdownHandler);
