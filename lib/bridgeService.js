...
class BridgeService {
    ...
    constructor(api, pluginManager, externalPortService, bridgeOptions, bridgeConfig) {
        ...
        this.storageService = new storageService_js_1.StorageService(this.bridgeOptions.cachedAccessoriesDir);
        ...
        this.bridge.on(hap_nodejs_1.AccessoryEventTypes.CHARACTERISTIC_WARNING, () => {
    ...
    static printCharacteristicWriteWarning(plugin, accessory, opts, warning) {
        ...
        switch (warning.type) {
            case hap_nodejs_1.CharacteristicWarningType.SLOW_READ:
            case hap_nodejs_1.CharacteristicWarningType.SLOW_WRITE:
                if (!opts.ignoreSlow) {
                    log.info((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), 'This plugin slows down Homebridge.', warning.message, wikiInfo);
                }
                break;
            case hap_nodejs_1.CharacteristicWarningType.TIMEOUT_READ:
            case hap_nodejs_1.CharacteristicWarningType.TIMEOUT_WRITE:
                log.error((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), 'This plugin slows down Homebridge.', warning.message, wikiInfo);
                break;
            case hap_nodejs_1.CharacteristicWarningType.WARN_MESSAGE:
                log.info((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), `This plugin generated a warning from the characteristic '${warning.characteristic.displayName}':`, `${warning.message}.`, wikiInfo);
                break;
            case hap_nodejs_1.CharacteristicWarningType.ERROR_MESSAGE:
                log.error((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), `This plugin threw an error from the characteristic '${warning.characteristic.displayName}':`, `${warning.message}.`, wikiInfo);
                break;
            case hap_nodejs_1.CharacteristicWarningType.DEBUG_MESSAGE:
                log.debug((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), `Characteristic '${warning.characteristic.displayName}':`, `${warning.message}.`, wikiInfo);
                break;
        ...
        if (warning.stack) {
            log.debug((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), warning.stack);
        }
    }
    publishBridge() {
        ...
        info.setCharacteristic(hap_nodejs_1.Characteristic.FirmwareRevision, bridgeConfig.firmwareRevision || (0, version_js_1.default)());
        this.bridge.on(hap_nodejs_1.AccessoryEventTypes.LISTENING, (port) => {
            log.success('Homebridge v%s (HAP v%s) (%s) is running on port %s.', (0, version_js_1.default)(), (0, hap_nodejs_1.HAPLibraryVersion)(), bridgeConfig.name, port);
        });
        const publishInfo = {
            username: bridgeConfig.username,
            port: bridgeConfig.port,
            pincode: bridgeConfig.pin,
            category: hap_nodejs_1.Categories.BRIDGE,
    ...
    async loadCachedPlatformAccessoriesFromDisk() {
        ...
        if (cachedAccessories) {
            ...
            this.cachedPlatformAccessories = cachedAccessories.map((serialized) => {
                return platformAccessory_js_1.PlatformAccessory.deserialize(serialized);
            });
    ...
    restoreCachedPlatformAccessories() {
        this.cachedPlatformAccessories = this.cachedPlatformAccessories.filter((accessory) => {
            ...
            if (plugin) {
                accessory._associatedHAPAccessory.on(hap_nodejs_1.AccessoryEventTypes.CHARACTERISTIC_WARNING, BridgeService.printCharacteristicWriteWarning.bind(this, plugin, accessory._associatedHAPAccessory, {}));
            }
            if (!platformPlugins) {
            ...
            try {
                ...
                log.warn(`${accessory._associatedPlugin ? (0, logger_js_1.getLogPrefix)(accessory._associatedPlugin) : ''} Could not restore cached accessory '${accessory._associatedHAPAccessory.displayName}':`, error.message);
                return false; // filter it from the list
            }
    ...
    saveCachedPlatformAccessoriesOnDisk() {
        try {
            ...
            if (this.cachedAccessoriesFileLoaded) {
                const serializedAccessories = this.cachedPlatformAccessories.map(accessory => platformAccessory_js_1.PlatformAccessory.serialize(accessory));
    ...
    async handlePublishExternalAccessories(accessories) {
        ...
        for (const accessory of accessories) {
            ...
            const advertiseAddress = (0, mac_js_1.generate)(hapAccessory.UUID);
            ...
            const plugin = this.pluginManager.getPlugin(accessory._associatedPlugin);
            if (plugin) {
                hapAccessory.on(hap_nodejs_1.AccessoryEventTypes.CHARACTERISTIC_WARNING, BridgeService.printCharacteristicWriteWarning.bind(this, plugin, hapAccessory, { ignoreSlow: true }));
            }
            else if (pluginManager_js_1.PluginManager.isQualifiedPluginIdentifier(accessory._associatedPlugin)) {
            ...
            hapAccessory.on(hap_nodejs_1.AccessoryEventTypes.LISTENING, (port) => {
    ...
    createHAPAccessory(plugin, accessoryInstance, displayName, accessoryType, uuidBase) {
        ...
        if (accessoryInstance.identify) {
            accessory.on(hap_nodejs_1.AccessoryEventTypes.IDENTIFY, (paired, callback) => {
        ...
        services.forEach((service) => {
            ...
            if (service instanceof hap_nodejs_1.Service.AccessoryInformation) {
                ...
                service.getCharacteristic(hap_nodejs_1.Characteristic.Identify).removeAllListeners(hap_nodejs_1.CharacteristicEventTypes.SET);
        ...
        accessory.on(hap_nodejs_1.AccessoryEventTypes.CHARACTERISTIC_WARNING, BridgeService.printCharacteristicWriteWarning.bind(this, plugin, accessory, {}));
    ...
    async loadPlatformAccessories(plugin, platformInstance, platformType, logger) {
        ...
        return new Promise((resolve) => {
            ...
            const loadDelayWarningInterval = setInterval(() => {
                log.warn((0, logger_js_1.getLogPrefix)(plugin.getPluginIdentifier()), 'This plugin is taking long time to load and preventing Homebridge from starting. See https://homebridge.io/w/JtMGR for more info.');
            }, 20000);
